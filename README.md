...
- Subsequent runs are faster since already-downloaded images are skipped.
...
- Google sheets implementation? 
   -Would be nice to have, but I have no idea how to work with the sheets API.
- Checkboxes instead of T/F
   - Harder than it sounds. Excel has three types of checkboxes: old Form Controls, ActiveX controls, and the new native cell checkboxes added in Excel 365. The first two are floating objects drawn on top of cells, and openpyxl has no support for them at all. The new native ones store TRUE/FALSE in the cell, but Microsoft hasn't published the file format for them, so the only way to create them programmatically is to reverse-engineer what Excel writes. Tried this and it didn't work reliably. T/F with the green row highlight is the closest you can get without it.